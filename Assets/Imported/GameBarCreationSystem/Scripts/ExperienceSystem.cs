...
class ExperienceSystem : ScrollBarEssentials
{
    private int level = 1;
    public float health;
    public ExperienceSystem(Rect sb_dimen, bool vbar, Texture sb_bt, Texture st, float rot)
    ...
    public override int DetermineMaxVal(int level)
    {
        ...
        return (int)(health/2 * level +(level * Mathf.Exp(Mathf.Pow(level, .333f))) * Mathf.Log(Mathf.Pow(level, .333f)));
    }
