...
public IEnumerator  ChoseNewAnimationTargetCoroutine (){
	while (true) {
...
void  Update (){
	...
	//	GetComponent<Renderer>().material.mainTextureScale.x = 0.1f * (hitInfo.distance);
        GetComponent<Renderer>().material.mainTextureScale += new Vector2(0.1f * (hitInfo.distance), 0.0f);
		...
		if (pointer) {
            ...
            pointer.transform.eulerAngles += new Vector3(90.0f,0.0f, 0.0f);
		}
	}
	else {
		if (pointer)
		...
		//GetComponent<Renderer>().material.mainTextureScale.x = 0.1f * (maxDist);
        GetComponent<Renderer>().material.mainTextureScale += new Vector2(0.1f * (maxDist), 0.0f);
